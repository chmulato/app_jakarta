...
def check_maven(args):
    ...
    if cp.returncode != 0:
        ...
        if cp.returncode != 0:
            ...
            if not props.exists():
                err("Maven não encontrado (global) e wrapper indisponível ou falhou.", args)
                return
            ...
            if not m_dist:
                err("Sem distributionUrl para bootstrap Maven.", args)
                return
...
def ensure_admin(args):
    ...
    if status.data["AdminCount"] > 0:
        ...
        return
    info("Criando usuário ADMIN default (ensure-admin)...", args)
    cp_col = run_cmd(["docker", "exec", POSTGRES_CONTAINER, "psql", "-U", POSTGRES_USER, "-d", POSTGRES_DB, "-t", "-A", "-c", "SELECT 1 FROM information_schema.columns WHERE table_name='usuarios' AND column_name='perfil' LIMIT 1;"])
    has_perfil = (cp_col.returncode == 0 and cp_col.stdout.strip() == '1')
    if not has_perfil:
